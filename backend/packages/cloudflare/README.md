...
Worker secrets: `JWT_SECRET`, `SECRET_KEY`, `CAST_SERVER_SECRET`, `CAST_CONTAINER_SECRET`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `PREVIEW_SECRET`. Use independent random values of at least 32 characters for signing/encryption secrets. R2 S3 credentials must be restricted to the checkpoint bucket. The account and bucket name are nonsecret configuration. Hosted agents use `env.AI` with `HOSTED_ENGINE=workers-ai`; no model key is required. `WORKERS_AI_MODEL` selects Qwen 3.8 27B. See [runtime adapter](../../../docs/workers-ai-runtime.md) for protocol, limits and recovery. `ANTHROPIC_API_KEY` remains an optional fallback for the legacy Claude engine.
